...
def _run(coro):
    """Run an async function synchronously for use in pytest without pytest-asyncio."""
...
class TestKeywordFallbackClassification:
    ...
    def test_empty_activities_returns_empty_result(self):
        result = _run(classify_assets([]))
        assert result.classifications == []
        assert result.skipped == []
        assert result.batch_tokens_used == 0

    def test_crane_keyword_match_returns_medium_confidence(self):
        activities = [_activity("a1", "Install precast wall panels")]
        result = _run(classify_assets(activities))
    ...
    def test_batch_tokens_used_is_zero_in_fallback(self):
        activities = [_activity("a1", "Crane lift operation")]
        result = _run(classify_assets(activities))
        assert result.batch_tokens_used == 0

    def test_multiple_crane_activities_all_classified(self):
    ...
    def test_project_asset_scoping_restricts_keywords(self):
        ...
        result = _run(classify_assets(activities, project_assets=project_assets))
        matched_ids = {c.activity_id for c in result.classifications}
        assert "a1" not in matched_ids, "crane keyword should be filtered (not on project)"
        assert "a2" in matched_ids

    def test_project_asset_scoping_allows_matching_types(self):
    ...
    def test_install_bd_reo_not_classified_as_crane(self):
        """
